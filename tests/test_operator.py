...
def perform(operator, payload):
    ...
    assert (isinstance(operator, BaseOperation)), "Operators must inherit from BaseOperation" #nosec
    ...
    assert (type(result).__name__ == 'list'), "Operators must return a list" #nosec

    # the list must be comprised of Message or None
    for item in result:
        assert(isinstance(item, Message) or item is None), "Each result must be a Message (or None)" #nosec
