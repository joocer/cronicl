...
class ExtractTweetDetailsStage(cronicl.stages.Stage):
...
class ExtractCVEsStage(cronicl.stages.Stage):
...
cronicl.Trace().setHandler(cronicl.StackDriverTracer('cronicl_trace'))


pipeline = nx.DiGraph()
pipeline.add_node('column_extract', function=ExtractTweetDetailsStage())
pipeline.add_node('cve_extract', function=ExtractCVEsStage())
pipeline.add_node('print', function=cronicl.stages.ScreenSink())
pipeline.add_node('bigquery', function=cronicl.stages.BigQuerySink(project='vulnerability-analytics',dataset='streaming',table='twitter_cve_table'))
